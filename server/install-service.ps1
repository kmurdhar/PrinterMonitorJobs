...
$printListenerLines = @(
    ...
    "        try {",
    "            `$events = Get-WinEvent -FilterHashtable @{LogName='Microsoft-Windows-PrintService/Operational'; ID=307} -MaxEvents 5 -ErrorAction SilentlyContinue | Where-Object { `$_.TimeCreated -gt (Get-Date).AddMinutes(-1) }",
    "        } catch {",
    "            `$events = `$null",
    "        }",
    "        ",
    ...
    "                Write-Log `"📋 Event log message: `$message`"",
    "                ",
    "                # Multiple patterns to match different Windows versions",
    "                if (`$message -match `"Document (.+?) owned by (.+?) was printed on (.+?) through port`" -or ",
    "                    `$message -match `"Document (.+?) owned by (.+?) was printed on (.+?)`" -or",
    "                    `$message -match `"Print Document (.+?) owned by (.+?) on printer (.+?)`") {",
    "                    `$fileName = `$matches[1]",
    "                    `$userName = `$matches[2]",
    "                    `$printerName = `$matches[3]",
    "                    `$systemName = `$env:COMPUTERNAME",
    "                    ",
    "                    # Try to extract page count from message",
    "                    `$pages = 1",
    "                    if (`$message -match `"(\d+) page`") {",
    "                        `$pages = [int]`$matches[1]",
    "                    }",
    "                    ",
    "                    `$fileSize = `"Unknown`"",
    "                    ",
    "                    Write-Log `"📄 Real print event detected: `$fileName by `$userName on `$printerName (`$pages pages)`"",
    "                    Send-PrintJob -FileName `$fileName -SystemName `$systemName -PrinterName `$printerName -Pages `$pages -FileSize `$fileSize -UserName `$userName",
    "                }",
    "            }",
    "        }",
